...
FIRE = '@'
EMPTY = ' '
WATER = '~'  # New water character



# Lake dimensions and position
LAKE_X = WIDTH // 2 - 5  # Center of lake
LAKE_Y = HEIGHT // 3 - 3
# (!) Change these settings to set how long and wide the lake will be.
LAKE_WIDTH = 10
LAKE_HEIGHT = 10
                ...
                if ((forest[(x, y)] == EMPTY)
                    ...
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            ...
                            if (forest.get((x + ix, y + iy)) == TREE and 
                                not any(forest.get((x + dx, y + dy)) == WATER #Fire cannot cross over the water
                                    for dx, dy in [(0,0), (ix,0), (0,iy), (ix,iy)])):                         
...
def createNewForest():
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            ...
            if (LAKE_X <= x < LAKE_X + LAKE_WIDTH and 
                LAKE_Y <= y < LAKE_Y + LAKE_HEIGHT):
                forest[(x, y)] = WATER
